...
async function createOrder(session) {
    ...
    const res = await axios_1.default.post(`${BASE_URL}/orders`, payload, {
        headers: {
...
// ---- Main Flow ---- //
const handleMessage = async (phone, msg) => {
    console.log("msg is here ", msg);
    let session = await (0, sessionStore_1.getSession)(phone);
    if (!session) {
        session = { step: "START" };
    }
    ...
    if (session.step === "START" && session.auth?.token) {
        ...
        await (0, sessionStore_1.setSession)(phone, session);
        await (0, metaService_1.sendMessage)(phone, "📍 Let's start with your pickup.\n\nYou can:\n• Type your address\n• Or share your 📌 *live location* using WhatsApp's location feature\n\nExample: Office 45, Nile Street, Khartoum, Sudan");
        return;
    }
    ...
    if (msgText === "menu" &&
        ...
        await (0, sessionStore_1.setSession)(phone, session);
        await (0, metaService_1.sendMessage)(phone, "📍 Let's book a ride! \n Let's start with your pickup.\n\nYou can:\n• Type your address\n• Or share your 📌 *live location* using WhatsApp's location feature\n\nExample: Office 45, Nile Street, Khartoum, Sudan\n\nExample: Office 45, Nile Street, Khartoum, Sudan");
        return;
    }
    ...
    if (msgText === "back" &&
        ...
        if (session.step === "ASK_DROP" || session.step === "SELECT_DROP") {
            session = { step: "ASK_PICKUP", auth: session.auth };
            await (0, sessionStore_1.setSession)(phone, session);
            await (0, metaService_1.sendMessage)(phone, "🔄 Okay, let's redo your pickup.\n\n📍 Type your address or share 📌 *live location*.\n\nExample: Office 45, Nile Street, Khartoum, Sudan");
            return;
        }
        if (session.step === "ASK_VEHICLE") {
            ...
            session.drop = null;
            await (0, sessionStore_1.setSession)(phone, session);
            await (0, metaService_1.sendMessage)(phone, "🔄 Okay, let's redo your destination.\n\n📍 Type your address or share 📌 *live location*.\n\nExample: Al Gomhuriya Street, Khartoum, Sudan");
            return;
        }
        if (session.step === "CONFIRM") {
            session.step = "ASK_VEHICLE";
            await (0, sessionStore_1.setSession)(phone, session);
            await (0, metaService_1.sendVehicleList)(phone, session.vehicleTypes);
            return;
        }
    }
    try {
        switch (session.step) {
            // ---------------- START ---------------- //
            case "START":
                await (0, authService_1.login)(phone);
                session.step = "ASK_OTP";
                await (0, metaService_1.sendMessage)(phone, "*Welcome TO Ride App*\nEnter OTP to continue:");
                break;
            // ---------------- OTP ---------------- //
            case "ASK_OTP":
                if (!msg?.text?.body) {
                    await (0, metaService_1.sendMessage)(phone, "Please enter the OTP sent to your number:");
                    return;
                }
                const auth = await (0, authService_1.verify)(phone, msg.text.body);
                session.auth = auth;
                session.step = "ASK_PICKUP";
                await (0, metaService_1.sendMessage)(phone, "📍 Let’s start with your pickup.\n\nYou can:\n• Type your address\n• Or share your 📌 *live location* using WhatsApp’s location feature\n\nExample: Office 45, Nile Street, Khartoum, Sudan");
                break;
            ...
            case "ASK_PICKUP":
                if (isLocationMessage(msg)) {
                    ...
                    await (0, metaService_1.sendMessage)(phone, "🔍 Locating your address...");
                    const geocoded = await (0, placesService_1.reverseGeocode)(latitude, longitude);
                    session.pickup = geocoded
                        ...
                        : extractLocation(msg);
                    session.step = "ASK_DROP";
                    await (0, metaService_1.sendMessage)(phone, `✅ Pickup set to:\n${session.pickup.address}\n\nNow enter your destination or share 📌 location.\n(Wrong pickup? Type *back* to redo it.)\n\nExample: Al Gomhuriya Street, Khartoum, Sudan`);
                }
                else {
                    const pickupResults = await (0, placesService_1.searchPlaces)(msg.text.body);
                    if (!pickupResults.length) {
                        await (0, metaService_1.sendMessage)(phone, "❌ No locations found. Please try a different address.");
                        return;
                    }
                    session.pickupResults = pickupResults;
                    session.step = "SELECT_PICKUP";
                    await (0, metaService_1.sendPlacesList)(phone, pickupResults, "📍 Choose Pickup");
                }
                break;
            // ---------------- SELECT PICKUP ---------------- //
            case "SELECT_PICKUP": {
                ...
                if (pickupId === "place_none") {
                    session.step = "ASK_PICKUP";
                    await (0, metaService_1.sendMessage)(phone, "📍 Please enter your pickup address again:");
                    break;
                }
                ...
                if (!pickupId && msg?.text?.body) {
                    const newPickupResults = await (0, placesService_1.searchPlaces)(msg.text.body);
                    if (!newPickupResults.length) {
                        await (0, metaService_1.sendMessage)(phone, "❌ No locations found. Please try a different address.");
                        return;
                    }
                    session.pickupResults = newPickupResults;
                    await (0, metaService_1.sendPlacesList)(phone, newPickupResults, "📍 Choose Pickup");
                    break;
                }
                ...
                if (!pickupPlace || isNaN(pickupIdx)) {
                    await (0, metaService_1.sendPlacesList)(phone, session.pickupResults, "📍 Choose Pickup");
                    break;
                }
                session.pickup = {
                ...
                session.pickupResults = null;
                session.step = "ASK_DROP";
                await (0, metaService_1.sendMessage)(phone, "Where would you like to go? 📍 Enter your destination.\n(Wrong pickup? Type *back* to redo it.)\n\nExample: Al Gomhuriya Street, Khartoum, Sudan");
                break;
            }
            // ---------------- DROP ---------------- //
            case "ASK_DROP":
                if (isLocationMessage(msg)) {
                    ...
                    await (0, metaService_1.sendMessage)(phone, "🔍 Locating your destination...");
                    const dropGeocoded = await (0, placesService_1.reverseGeocode)(dropLat, dropLng);
                    session.drop = dropGeocoded
                        ? {
                            address: dropGeocoded.address,
                            coordinates: dropGeocoded.coordinates,
                            scheduleDateAfter: 0,
                            scheduleDateBefore: 0,
                        }
                        : extractLocation(msg);
                    // fetch vehicle types
                    const vehiclesDirect = await getVehicleTypes(session.auth.token, session.auth.customerId, session.pickup.coordinates);
                    ...
                    const dropResults = await (0, placesService_1.searchPlaces)(msg.text.body);
                    ...
                    await (0, metaService_1.sendPlacesList)(phone, dropResults, "📍 Choose Dropoff");
                }
                break;
            // ---------------- SELECT DROP ---------------- //
            case "SELECT_DROP": {
                ...
                if (isLocationMessage(msg)) {
                    ...
                    const vehiclesFromLocation = await getVehicleTypes(session.auth.token, session.auth.customerId, session.pickup.coordinates);
                ...
                if (dropId === "place_none") {
                    session.step = "ASK_DROP";
                    await (0, metaService_1.sendMessage)(phone, "📍 Please enter your destination address again:");
                    break;
                }
                // No valid list selection — if the user typed free text instead of
                ...
                if (!dropId && msg?.text?.body) {
                    const newDropResults = await (0, placesService_1.searchPlaces)(msg.text.body);
                    if (!newDropResults.length) {
                        await (0, metaService_1.sendMessage)(phone, "❌ No locations found. Please try a different address.");
                        return;
                    }
                    session.dropResults = newDropResults;
                    await (0, metaService_1.sendPlacesList)(phone, newDropResults, "📍 Choose Dropoff");
                    break;
                }
                const dropIdx = parseInt(dropId?.split("_")[1]);
                const dropPlace = session.dropResults?.[dropIdx];
                if (!dropPlace || isNaN(dropIdx)) {
                    await (0, metaService_1.sendPlacesList)(phone, session.dropResults, "📍 Choose Dropoff");
                    break;
                }
                session.drop = {
                ...
                session.dropResults = null;
                const vehicles = await getVehicleTypes(session.auth.token, session.auth.customerId, session.pickup.coordinates);
            ...
            case "ASK_VEHICLE":
                ...
                await (0, metaService_1.sendConfirmButtons)(phone, session.pickup.address, session.drop.address, selected.title);
                break;
            // ---------------- CONFIRM ---------------- //
            case "CONFIRM":
                ...
                if (!reply) {
                    ...
                    await (0, metaService_1.sendConfirmButtons)(phone, session.pickup.address, session.drop.address, session.selectedVehicleTitle);
                    return;
                }
                if (reply === "confirm_no") {
                    await (0, metaService_1.sendMessage)(phone, "❌ Booking cancelled.");
                    session = { step: "ASK_PICKUP", auth: session.auth };
                    await (0, metaService_1.sendMessage)(phone, "📍 Let's start with your pickup.\n\nYou can:\n• Type your address\n• Or share your 📌 *live location* using WhatsApp's location feature\n\nExample: Office 45, Nile Street, Khartoum, Sudan");
                    break;
                }
                await (0, metaService_1.sendMessage)(phone, "⏳ Booking your ride...");
                ...
                if (trackUrl)
                    successMsg += `\n🔗 Track: ${trackUrl}`;
                await (0, metaService_1.sendMessage)(phone, successMsg);
                // Keep auth, reset to pickup for next ride
                session = { step: "ASK_PICKUP", auth: session.auth };
                await (0, metaService_1.sendMessage)(phone, "📍 Need another ride? Enter your pickup location or type *menu* to restart.");
                break;
        }
        if (session) {
            await (0, sessionStore_1.setSession)(phone, session);
            ...
            await (0, sessionStore_1.deleteSession)(phone);
        }
    }
    catch (err) {
        ...
        if (status === 401) {
            ...
            await (0, authService_1.login)(phone);
            await (0, sessionStore_1.deleteSession)(phone);
            await (0, metaService_1.sendMessage)(phone, "🔐 Your session expired. A new OTP has been sent — please enter it:");
            await (0, sessionStore_1.setSession)(phone, { step: "ASK_OTP" });
        }
        else {
            await (0, metaService_1.sendMessage)(phone, "⚠️ Something went wrong. Please try again.");
            // Reset to pickup but keep auth so user doesn't need to re-verify OTP
            const savedAuth = session?.auth;
            if (savedAuth) {
                await (0, sessionStore_1.setSession)(phone, { step: "ASK_PICKUP", auth: savedAuth });
            }
            else {
                await (0, sessionStore_1.deleteSession)(phone);
            }
